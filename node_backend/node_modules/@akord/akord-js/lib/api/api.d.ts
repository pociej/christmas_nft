...
declare abstract class Api {
    config: any;
    constructor();
    abstract postContractTransaction<T>(vaultId: string, input: ContractInput, tags: Tags, metadata?: any): Promise<{
        id: string;
        object: T;
    }>;
    abstract initContractId(tags: Tags, state?: any): Promise<string>;
    abstract uploadFile(file: ArrayBuffer, tags: Tags, options?: FileUploadOptions): Promise<{
        resourceTx: string;
        resourceUrl: string;
    }>;
    abstract uploadData(items: {
        data: any;
        tags: Tags;
    }[], options?: FileUploadOptions): Promise<Array<string>>;
    abstract getContractState(vaultId: string): Promise<ContractState>;
    abstract downloadFile(id: string, options?: FileDownloadOptions): Promise<{
    ...
    abstract inviteNewUser(vaultId: string, email: string, role: string, message?: string): Promise<{
        id: string;
    }>;
    abstract revokeInvite(vaultId: string, membershipId: string): Promise<{
        id: string;
    }>;
    abstract inviteResend(vaultId: string, membershipId: string): Promise<{
        id: string;
    }>;
}
export { Api };
