...
const cacheable_decorator_2 = require("../cacheable.decorator");
...
strategies.forEach(s => {
    ...
    describe(`${!s ? 'InMemoryStorageStrategy' : s.name}: CacheableDecorator`, () => {
        ...
        const cacheModifier = new rxjs_2.Subject();
        beforeEach(() => {
            const cacheBusterNotifier = new rxjs_2.Subject();
            class Service {
        ...
        it('return cached data for 5 unique requests, then should bail to data source', () => {
            ...
            parameters.forEach((param) => __awaiter(void 0, void 0, void 0, function* () { return _timedStreamAsyncAwait(service.getDataWithMaxCacheCount(param), 1000); }));
            /**
             ...
             */
            const cachedResponseAll = _timedStreamAsyncAwait((0, rxjs_1.forkJoin)(parameters.map(param => service.getDataWithMaxCacheCount(param))));
             ...
             */
            const cachedResponseAll2 = _timedStreamAsyncAwait((0, rxjs_1.forkJoin)(newParameters.map(param => service.getDataWithMaxCacheCount(param))), 1000);
        ...
        it('return cached data for 5 unique requests all available for 7500ms', () => {
            ...
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
            jasmine.clock().tick(1000);
            const cachedResponse2 = _timedStreamAsyncAwait((0, rxjs_1.forkJoin)(parameters.map(param => service.getDataWithMaxCacheCountAndExpiration(param))));
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
        ...
        it('return cached data for 5 unique requests all available for 7500ms WITH slidingExpiration on', () => {
            ...
            const cachedResponse = _timedStreamAsyncAwait((0, rxjs_1.combineLatest)(parameters.map(param => service
        ...
        it('return cached data up until new parameters are passed WITH a custom resolver function', () => {
            ...
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(2);
            const cachedResponse = _timedStreamAsyncAwait(service.getDataWithCustomCacheResolver('test3', {
        ...
        it('should call a function with a complex instance which should not be touched and passed to the original method as it is', () => {
            ...
            const response = _timedStreamAsyncAwait(service.getWithAComplexType(complexObject), 1000);
        ...
        it('should clear all caches when the global cache buster is called', () => {
            ...
            const asyncFreshData1 = _timedStreamAsyncAwait(service.getData('test1'), 1000);
            ...
            const asyncFreshData2 = _timedStreamAsyncAwait(service.getData('test2'), 1000);
            expect(asyncFreshData2).toEqual({ payload: 'test2' });
            const cachedResponse2 = _timedStreamAsyncAwait(service.getData('test2'));
            expect(cachedResponse2).toEqual({ payload: 'test2' });
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(2);
            service.getData('test3');
            const asyncFreshData3 = _timedStreamAsyncAwait(service.getData('test3'), 1000);
        ...
        it('should not change undefined parameters to null', () => {
            ...
            const asyncData = _timedStreamAsyncAwait(service.getDataWithMultipleUndefinedParameters(undefined, undefined), 1000);
        ...
        it('should work correctly with a custom storage strategy', () => {
            const addSpy = spyOn(InMemoryStorageStrategy_1.InMemoryStorageStrategy.prototype, 'add').and.callThrough();
            const updateSpy = spyOn(InMemoryStorageStrategy_1.InMemoryStorageStrategy.prototype, 'update').and.callThrough();
            const getAllSpy = spyOn(InMemoryStorageStrategy_1.InMemoryStorageStrategy.prototype, 'getAll').and.callThrough();
            const removeSpy = spyOn(InMemoryStorageStrategy_1.InMemoryStorageStrategy.prototype, 'remove').and.callThrough();
            const removeAllSpy = spyOn(InMemoryStorageStrategy_1.InMemoryStorageStrategy.prototype, 'removeAll').and.callThrough();
            jasmine.clock().mockDate();
            const asyncFreshData = _timedStreamAsyncAwait(service.getDataWithCustomStorageStrategyProvided('test'), 1000);
            ...
            const cachedResponse = _timedStreamAsyncAwait(service.getDataWithCustomStorageStrategyProvided('test'));
             ...
             * this should have returned null, if the cache didnt renew
             */
            const cachedResponse2 = _timedStreamAsyncAwait(service.getDataWithCustomStorageStrategyProvided('test'));
            // this call will renew the updateAtIndex call count since it's used to renew the cache
             ...
             */
            jasmine.clock().tick(7501);
            const cachedResponse3 = _timedStreamAsyncAwait(service.getDataWithCustomStorageStrategyProvided('test'));
        ...
        it('use the maxAge and slidingExpiration from the GlobalCacheConfig', () => {
             ...
             * this should have returned null, if the cache didnt renew
             */
            const cachedResponse2 = _timedStreamAsyncAwait(service.getData('test'));
            expect(cachedResponse2).toEqual({ payload: 'test' });
            /**
             * call count is still one, because we renewed the cache 4501ms ago
             */
             ...
             */
            jasmine.clock().tick(7501);
            const cachedResponse3 = _timedStreamAsyncAwait(service.getData('test'));
            /**
             * cached has expired, request hasn't returned yet but still - the service was called
             */
            expect(cachedResponse3).toEqual(null);
        ...
        it('use the maxCacheCount from the GlobalCacheConfig', () => {
             ...
             */
            const parameters = ['test1', 'test2', 'test3', 'test4', 'test5'];
            parameters.forEach((param) => __awaiter(void 0, void 0, void 0, function* () { return _timedStreamAsyncAwait(service.getData(param), 1000); }));
            /**
             * data for all endpoints should be available through cache by now
             */
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
            const cachedResponse = _timedStreamAsyncAwait(service.getData('test1'));
            expect(cachedResponse).toEqual({ payload: 'test1' });
            /** call count still 5 */
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
            /**
             * this should return a maximum of 5 different cached responses
             */
            const cachedResponseAll = _timedStreamAsyncAwait((0, rxjs_1.forkJoin)(parameters.map(param => service.getData(param))));
            expect(cachedResponseAll).toEqual([
                { payload: 'test1' },
                { payload: 'test2' },
                { payload: 'test3' },
            ...
            /** call count still 5 */
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
            const asyncData = _timedStreamAsyncAwait(service.getData('test6'), 1000);
            expect(asyncData).toEqual({ payload: 'test6' });
            /** call count incremented by one */
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(6);
            /**
             ...
             * this should return a maximum of 5 different cached responses, with the latest one in the end
             */
            const cachedResponseAll2 = _timedStreamAsyncAwait((0, rxjs_1.forkJoin)(newParameters.map(param => service.getData(param))), 1000);
            expect(cachedResponseAll2).toEqual([
                { payload: 'test2' },
                { payload: 'test3' },
                { payload: 'test4' },
             ...
             * fetch and cache the test7 response
             */
            const nonCachedResponse = _timedStreamAsyncAwait(service.getData('test7'), 1000);
            expect(nonCachedResponse).toEqual({ payload: 'test7' });
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(7);
            /**
             * since the cached response for 'test2' was now removed from cache by 'test7', it shouldn't be available in cache
        ...
        it('should modify cache of getMutableData dynamically', () => {
            const asyncFreshData = _timedStreamAsyncAwait(service.getMutableData('test'), 1000);
            expect(asyncFreshData).toEqual({ payload: 'test' });
            expect(mockServiceCallSpy).toHaveBeenCalledTimes(1);
